...
Select 'EDI docs: ', LEFT(CONVERT(varchar, d.CreatedTimestamp,112),6) As YearMonth, AVG(DATEDIFF(MILLISECOND, d.CreatedTimestamp, d.ChangedTimestamp)) as DiffMs, COUNT(*) as NumberOfDocs
...
Select 'Expense docs: ', LEFT(CONVERT(varchar, d.CreatedTimestamp,112),6) As YearMonth, AVG(DATEDIFF(MILLISECOND, d.CreatedTimestamp, task.Created)) as DiffMs, COUNT(*) as NumberOfDocs
...
Select 'Match docs: ', LEFT(CONVERT(varchar, d.CreatedTimestamp,112),6) As YearMonth, AVG(DATEDIFF(MILLISECOND, d.CreatedTimestamp, task.Created)) as DiffMs, COUNT(*) as NumberOfDocs
/*d.Id as DocumentId, d.CreatedTimestamp, d.ChangedTimestamp, task.Created, */
from [Medius.Data].Document d Inner Join [Medius.PurchaseToPay.Entities].SupplierInvoice si On(si.Document_id = d.Id)
